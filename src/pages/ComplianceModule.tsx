...
import { ComplianceAudit, ComplianceControl, ComplianceStandard } from "@/types";
import { getGDPRControls, getISO27001Controls, saveComplianceResults } from "@/lib/api-utils";
...
const ComplianceModule = () => {
  ...
  // Load controls for the selected standard
  const loadControls = (standard: ComplianceStandard) => {
    setSelectedStandard(standard);
    
    if (standard === 'GDPR') {
      setControls(getGDPRControls());
    } else if (standard === 'ISO27001') {
      setControls(getISO27001Controls());
    }
  };
  ...
  // Complete the audit and generate results
  const completeAudit = async () => {
    ...
    // Save to Supabase
    try {
      await saveComplianceResults(results);
    } catch (error) {
          ...
          <Card className="p-6">
            ...
            <div className="mt-8 flex justify-end">
              <Button 
                ...
                disabled={loading || controls.some(c => c.status === 'N/A')}
              >
                {loading ? (
                  <>
